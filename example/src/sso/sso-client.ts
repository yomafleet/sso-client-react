...
import { createQueryParams, parseAuthenticationResult } from "./utils/utils";
...
export default class SSOClient {
  ...
  async handleRedirectCallback(
    ...
    const { state, code, error } = parseAuthenticationResult(
    ...
    await this._requestToken(code as string);
  }
   ...
   * @return Promise<void>
   */
  private async _requestToken(code: string): Promise<void> {
  ...
  getUser<TUser extends User>(): TUser {
    ...
    return jwtDecode(id_token as string);
  }
