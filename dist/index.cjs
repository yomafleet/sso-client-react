 ...
 */
const useAuth = (context = SSOContext) => React.useContext(context);
...
const SSOProvider = ({ domain, clientId, redirectUri, onRedirectCalbck, children, }) => {
    ...
    const [client] = React.useState(() => new SSOClient({
        domain,
        client_id: clientId,
    ...
    const loginWithRedirect = React.useCallback(() => client.loginWithRedirect(), [client]);
    const logout = React.useCallback(() => client.logout(), [client]);
    const contextValue = React.useMemo(() => (Object.assign(Object.assign({}, state), { loginWithRedirect, logout })), [state, loginWithRedirect, logout]);
